...
import { createNoise2D } from 'https://unpkg.com/simplex-noise@4.0.3/dist/esm/index.js';
...
function generateChunk(cx, cz) {
  ...
  const addFace = (verts, nx, ny, nz, color) => {
    for (let v of verts) positions.push(v[0], v[1], v[2]);
  ...
  for (let x = 0; x < CHUNK_SIZE; x++) {
    for (let z = 0; z < CHUNK_SIZE; z++) {
      for (let y = 0; y < CHUNK_HEIGHT; y++) {
        ...
        if (!neighbors.px) addFace(faceVerts.px.map(v => [v[0] + worldX, v[1] + y, v[2] + worldZ]), 1, 0, 0, color);
        if (!neighbors.nx) addFace(faceVerts.nx.map(v => [v[0] + worldX, v[1] + y, v[2] + worldZ]), -1, 0, 0, color);
        if (!neighbors.py) addFace(faceVerts.py.map(v => [v[0] + worldX, v[1] + y, v[2] + worldZ]), 0, 1, 0, color);
        if (!neighbors.ny) addFace(faceVerts.ny.map(v => [v[0] + worldX, v[1] + y, v[2] + worldZ]), 0, -1, 0, color);
        if (!neighbors.pz) addFace(faceVerts.pz.map(v => [v[0] + worldX, v[1] + y, v[2] + worldZ]), 0, 0, 1, color);
        if (!neighbors.nz) addFace(faceVerts.nz.map(v => [v[0] + worldX, v[1] + y, v[2] + worldZ]), 0, 0, -1, color);
      }
    }
  }
